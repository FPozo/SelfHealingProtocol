...
class Simulation:
    ...
    def __init__(self, network: Network):
        ...
        self.__path_activator_links: Dict[int, List[int]] = {}
        self.__path_leader_links: Dict[int, List[int]] = {}
        ...
        # Patched offsets per link broken and node who performed the patch
        self.__patched_offsets: Dict[int, Dict[int, List[Tuple, int, Frame, Offset]]] = {}
    ...
    def __simulate_internal_event(self, event: InternalEvent) -> None:
        ...
        if event.name is InternalEvent.InternalName.LinkFailure:
            ...
            try:
                ...
                self.__path_activator_links[event.event_id] = []
                for node_it, node in enumerate(self.__path_activator[event.event_id][:-1]):
                    self.__path_activator_links[event.event_id].append(
                        self.__network.get_link_data(node, self.__path_activator[event.event_id][node_it + 1])[1])
            except ValueError:
            ...
            try:
                ...
                self.__path_leader_links[event.event_id] = []
                self.__patched_offsets[event.event_id] = {}
                for node_it, node in enumerate(self.__path_leader[event.event_id][:-1]):
                    self.__patched_offsets[event.event_id][node] = []
                    self.__path_leader_links[event.event_id].append(
                        self.__network.get_link_data(node, self.__path_leader[event.event_id][node_it + 1])[1])
                self.__successful_heal[event.event_id] = False
            except ValueError:
                raise Simulation.NoPath('The link failure cannot be repaired as there is no path to connect')
    ...
    def __simulate_execution_event(self, event: ExecutionEvent, node_id: int) -> None:
        ...
        if event.name is ExecutionEvent.ExecutionName.Patch:
            ...
            if isfile(patched_file):
                ...
                # We do not change the patched schedule to facilitate optimization
    ...
    def __check_optimization_status(self, event_id: int, time: int) -> None:
        ...
        if self.__optimization_status[event_id] == (len(self.__path_activator[event_id]) - 1):
            ...
            # For every frame path, update the path avoiding loops, then eliminate all not used offsets
            for frame_id, frame in self.__network.frames.items():

                for receiver_id in frame.receivers_id:
                    path = list(frame.get_path(receiver_id))

                    # Remove the broken link if it is in the frame path
                    if event_id in path:
                        path.remove(event_id)

                        # Add the new path
                        not_loop_path = frame.get_path(receiver_id)
                        index_link = not_loop_path.index(event_id)
                        not_loop_path.remove(event_id)
                        not_loop_path[index_link:index_link] = self.__path_activator_links[event_id]

                        # Remove the loop if there exists one
                        receivers_paths = [self.__network.get_receiver_id_link(link) for link in path]
                        for new_link_path in self.__path_activator_links[event_id]:
    ...
    def __write_patch_traffic_xml(self, traffic: Xml.Element, broken_link_id: int, node_id: int) -> None:
        ...
        for frame_id, frame, offset in self.__network.get_offsets_by_link(broken_link_id):
            ...
            link, link_id = self.__network.get_link_data(node_id,
                                                         self.__path_activator[broken_link_id][position_path + 1])
            ...
            link, link_id = self.__network.get_link_data(node_id,
                                                         self.__path_activator[broken_link_id][position_path + 1])
            transmission_range = self.__network.get_atr(frame, offset, broken_link_id,
                                                        self.__path_activator_links[broken_link_id], link_id)
    ...
    def write_simulation_master_xml(self, simulation_file: str) -> None:
        ...
        :param simulation_file: path and name of the simulation folder
        :return: nothing
        """
